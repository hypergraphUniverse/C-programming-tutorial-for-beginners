...
int main(void){
	...
	while(1){
		...
		if(input==0){
			...
			printf("名:");scanf("%s",person[num].first_name);
			printf("姓:");scanf("%s",person[num].last_name);
			printf("电话:");scanf("%lld",&person[num].tele);
			printf("是否为VIP(1/0):");scanf("%d",&person[num].isVip);
			num++;
		}else if(input==3){
			int target;
			printf("删除的人在列表中第几位:");
